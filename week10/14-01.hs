...
test :: Tree Int
test = Tree 5 (Tree 10 EmptyTree EmptyTree) (Tree 3 EmptyTree EmptyTree)
...
subsequences :: [a] -> [[a]]
subsequences [] = []
subsequences xs = [] : [sufix | prefixes <- inits xs, sufix <- tails prefixes] --всяко за всяко за съответния бррой префикси правим и суфикси 
    where inits [] = [] 
...
-- Да се генерира поток sumsOfCubes от тези числа, които са сума от кубовете на 
--две положителни цели числа
 
 sumsOfCubes = [x^3 + y^3 | x <- [1..], y <- [1..]]

-- Път от корен до възел в двоично дърво кодираме с поредица от цифри 0 и 1, 
--която започва с цифрата 1, а за всяка следваща цифра 0 означава завиване по левия клон, 
--а 1 — по десния. Да се реализира функция sameAsCode, която в двоично дърво от числа връща
--такова число x, което съвпада по стойност с двоичното число, кодиращо пътя от корена до x,
--ли 0, ако такова число няма. Представянето на дървото е по ваш избор.
