...
(define (longer-tracks tracks)
  (define (check? xs)
    (if (>= (car(cdr(cdr (cdr (car  tracks))))) 240000)
        #t
        #f))
...
(longer-tracks tracks)

; Да вземем само имената на горните?

(define (names-of-longer-tracks tracks)
  (define (loop xs)
    (cond
      ((null? (cdr xs)) (car (car xs)))
      (else (cons (car(car xs)) (loop(cdr xs))))))
  (loop tracks)
)
(names-of-longer-tracks tracks)
;(car(car tracks))

; Да направим времетраенето на всякa в секунди
(define (duration-in-seconds tracks)
  ...
  (define (loop xs)
    (cond
      ((null? tracks) '())
      (else
       (let (newline (change (car xs))))
       (cons newline (loop (cdr xs)))
     )))
  (loop tracks)
)

; Да намерим най-популярната

(define (most-popular-track tracks)
  (void)
)

; Да намерим средната популярност 

(define (average-popularity tracks)
  (void)
)

; Всички с популярност под средната

(define (hipster-tracks tracks)
  (void)
)

; Всички с популярност над средната
